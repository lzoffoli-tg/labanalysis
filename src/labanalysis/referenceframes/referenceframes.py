...
class ReferenceFrame:
    ...
    def _extract_numpy_array(param, param_name: str) -> np.ndarray:
        ...
        if isinstance(param, Timeseries):
            ...
            return np.atleast_2d(param).astype(float)
        else:
            raise TypeError(
    ...
    def __init__(
        self,
        origin: np.ndarray | list | Timeseries,
        lateral_axis: np.ndarray | list | Timeseries | None = None,
        vertical_axis: np.ndarray | list | Timeseries | None = None,
        anteroposterior_axis: np.ndarray | list | Timeseries | None = None,
    ):
        """
        ...
        origin_np = self._extract_numpy_array(origin, "origin")

        if lateral_axis is None:
            if anteroposterior_axis is None or vertical_axis is None:
                raise ValueError("at least 2 vectors must be provided.")
            ax1 = self._extract_numpy_array(
                vertical_axis,
                "vertical_axis",
            )
            ...
            if isinstance(anteroposterior_axis, Timeseries):
                ...
                lateral_axis = np.cross(ax1, ax2)
        lateral_np = self._extract_numpy_array(
            lateral_axis,
            "lateral_axis",
        )

        if vertical_axis is None:
            if anteroposterior_axis is None or lateral_axis is None:
                raise ValueError("at least 2 vectors must be provided.")
            ax1 = self._extract_numpy_array(
                anteroposterior_axis,
                "anteroposterior_axis",
            ...
            if isinstance(anteroposterior_axis, Timeseries):
                ...
                vertical_axis = np.cross(ax1, ax2)
        vertical_np = self._extract_numpy_array(
            vertical_axis,
            "vertical_axis",
        )

        if anteroposterior_axis is None:
            if vertical_axis is None or lateral_axis is None:
                raise ValueError("at least 2 vectors must be provided.")
            ax1 = self._extract_numpy_array(
                lateral_axis,
                "lateral_axis",
            ...
            if isinstance(vertical_axis, Timeseries):
                ...
                anteroposterior_axis = np.cross(ax1, ax2)
        anteroposterior_np = self._extract_numpy_array(
            anteroposterior_axis,
            "anteroposterior_axis",
        )

        # Validate shapes - all must have 3 columns
        if origin_np.shape[1] != 3:
            raise ValueError(
                f"origin must have 3 columns, got {origin_np.shape[1]} columns"
            )
        if lateral_np.shape[1] != 3:
            raise ValueError(
                f"lateral_axis must have 3 columns, got {lateral_np.shape[1]} columns"
            )
        if vertical_np.shape[1] != 3:
            raise ValueError(
                f"vertical_axis must have 3 columns, got {vertical_np.shape[1]} columns"
            )
        if anteroposterior_np.shape[1] != 3:
            raise ValueError(
                f"anteroposterior_axis must have 3 columns, got {anteroposterior_np.shape[1]} columns"
            )

        # Validate all inputs have same number of rows
        n_samples = origin_np.shape[0]
        if lateral_np.shape[0] != n_samples:
            raise ValueError(
                f"All inputs must have same number of rows. "
                f"origin has {n_samples} rows, lateral_axis has {lateral_np.shape[0]} rows"
            )
        if vertical_np.shape[0] != n_samples:
            raise ValueError(
                f"All inputs must have same number of rows. "
                f"origin has {n_samples} rows, vertical_axis has {vertical_np.shape[0]} rows"
            )
        if anteroposterior_np.shape[0] != n_samples:
            raise ValueError(
                f"All inputs must have same number of rows. "
                f"origin has {n_samples} rows, vertical_axis has {anteroposterior_np.shape[0]} rows"
            )

        # Store inputs
        self._origin = origin_np
        self._lateral_axis = lateral_np
        self._vertical_axis = vertical_np
        self._anteroposterior_axis = anteroposterior_np
    ...
    def _apply_to_dataframe(self, df: pd.DataFrame, inplace: bool):
    ...
    def _apply_to_forceplatform(self, fp: ForcePlatform, inplace: bool):
        ...
        origin_broadcast = (
            np.broadcast_to(origin_np, (fp.origin.shape[0], 3))
            if self._is_single_frame
            else origin_np
        )
    ...
    def _apply_inverse_to_numpy(self, data: np.ndarray):
        ...
        if self._is_single_frame:
            ...
            origin = np.broadcast_to(origin_np, (n_data_samples, 3))
        else:
            # Multiple frames - must match exactly
            if n_data_samples != self._n_samples:
                raise ValueError(
    ...
    def _apply_inverse_to_dataframe(self, df: pd.DataFrame, inplace: bool):
    ...
    def _apply_inverse_to_forceplatform(self, fp: ForcePlatform, inplace: bool):
    ...
    def copy(self):
        ...
        return ReferenceFrame(
            ...
            anteroposterior_axis=(
                self._anteroposterior_axis.copy()
                if self._anteroposterior_axis is not None
                else None
            ),
        )
