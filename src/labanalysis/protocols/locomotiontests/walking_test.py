...
from ...records.locomotion import WalkingExercise
    ...
    **extra_signals : Signal1D, Signal3D, EMGSignal, Point3D, ForcePlatform
    ...
    def get_results(self):
        ...
        for i, cycle in enumerate(self.cycles):
            ...
            if res is None:
                continue
            cop = res["origin"].copy().reset_time(inplace=False).to_dataframe()  # type: ignore
            cop.insert(0, "Time", cop.index)
            cop.insert(0, "Cycle", i + 1)
            cop_list += [cop[horizontal_axes]]

            # add grf
            grf = res["force"].copy()[self.vertical_axis].reset_time().to_dataframe()  # type: ignore
