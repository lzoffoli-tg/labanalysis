...
from labanalysis.modelling.pytorch.modules import (
    FeaturesGenerator,
    BoxCoxTransform,
    SigmoidTransformer,
    PCA,
...
def test_boxcox_transform():
    ...
    bct = BoxCoxTransform(n_features=2)
    y = bct(x)
    x_inv = bct.inverse(y)
...
def test_sigmoid_transformer():
    x = torch.randn(5, 3)
    st = SigmoidTransformer(input_dim=3, output_dim=2, transform_dim=1)
    y = st(x)
    assert y.shape == (5, 2)
...
def test_pca():
    x = torch.randn(10, 5)
    pca = PCA(input_dim=5, output_dim=3)
    y = pca(x)
...
def test_lasso():
    x = torch.randn(8, 4)
    lasso = Lasso(in_features=4, out_features=2)
    y = lasso(x)
